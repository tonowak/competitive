...
void fft(valarray<Complex> &x, bool inverse) {
	int n = int(x.size()), n2 = n / 2;
	if(n == 1)
		return;

	valarray<Complex> even = x[slice(0, n2, 2)],
					  odd  = x[slice(1, n2, 2)];
	fft(even, inverse);
	fft(odd,  inverse);

	Complex w = 1, wn = polar(1.0, (inverse ? 1 : -1) * 2 * pi / n);
	REP(i, n2) {
		Complex v = even[i], u = odd[i] * w;
		x[i]      = v + u;
		x[i + n2] = v - u;
		w *= wn;
	}
...
vector<L> multiply(vector<int> &ai, vector<int> &bi) {
	int m = SZ(ai) + SZ(bi), n = ceil2(m);
	valarray<Complex> a(n), b(n);
	REP(i, SZ(ai))
		a[i] = ai[i];
	REP(i, SZ(bi))
		b[i] = bi[i];
